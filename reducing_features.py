...
# x = pd.DataFrame(StandardScaler().fit_transform(train_features.iloc[:, 5:]), columns=train_features.columns[5:])
...
# x = pd.DataFrame(StandardScaler().fit_transform(test_features.iloc[:, 5:]), columns=test_features.columns[5:])

#Perform PCA on the test features twice, once on the video, and once on the audio
